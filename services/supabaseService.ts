...
import { NewsItem, GalleryItem, VisitorLog, User, Department, ChatMessage, SiteSettings, TelegramInbox, ContactMessage, Teacher, Eskul, AgendaItem, Suggestion } from '../types';
import { createClient } from '@supabase/supabase-js';

// Vercel/Vite will inject process.env
...
export const telegramService = {
  ...
  broadcastToChannel: async (text: string, imageUrl?: string) => {
    const settings = await settingsService.get();
    const token = settings.telegram_bot_token || DEFAULT_TELEGRAM_TOKEN;
    const chatId = settings.telegram_chat_id || DEFAULT_TELEGRAM_CHAT_ID;
  ...
  activateAdminMenu: async () => {
    const msg = `<b>🎛️ PANEL KONTROL ADMIN AKTIF</b>\n\nSilakan gunakan tombol di bawah untuk memposting konten secara instan.`;
    const keyboard = {
      keyboard: [
        [{ text: "📢 Tambah Berita" }, { text: "📸 Tambah Galeri" }],
        [{ text: "🌐 Status Website" }, { text: "❓ Bantuan" }]
      ],
      resize_keyboard: true,
      one_time_keyboard: false
    };
    return await telegramService.sendNotification(msg, keyboard);
  },
  ...
  getFileUrl: async (fileId: string) => {
    ...
    try {
      ...
      if (data.ok) return `https://api.telegram.org/file/bot${token}/${data.result.file_path}`;
      return null;
...
export const settingsService = {
  get: async (): Promise<SiteSettings> => {
    const defaultSettings: SiteSettings = {
      ...
      sub_welcome_text: 'SMKN 2 Tembilahan berkomitmen menghadirkan pendidikan vokasi berkualitas tinggi yang terintegrasi dengan kebutuhan industri modern.',
      running_text: '',
...
export const eskulService = {
  getAll: async (): Promise<Eskul[]> => {
    if (supabase) {
      const { data } = await supabase.from('eskul').select('*').order('name', { ascending: true });
      return data || [];
    }
...
export const agendaService = {
  ...
  save: async (item: Omit<AgendaItem, 'id' | 'created_at'>): Promise<AgendaItem> => {
    ...
    mockService.set(LOCAL_STORAGE_KEYS.AGENDA, [newItem, ...all].sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime()));
    return newItem;
  },
...
export const contactService = {
  getAll: async (): Promise<ContactMessage[]> => {
    ...
    return mockService.get(LOCAL_STORAGE_KEYS.CONTACTS);
  },
  send: async (msg: Omit<ContactMessage, 'id' | 'created_at' | 'is_read'>): Promise<ContactMessage> => {
    const newItem = { 
      ...msg, 
    ...
    if (supabase) { await supabase.from('contacts').insert([newItem]); return newItem; }
    const all = mockService.get(LOCAL_STORAGE_KEYS.CONTACTS);
    mockService.set(LOCAL_STORAGE_KEYS.CONTACTS, [newItem, ...all]);
    return newItem;
  },
  delete: async (id: string): Promise<void> => {
    if (supabase) { await supabase.from('contacts').delete().eq('id', id); return; }
    const all = mockService.get(LOCAL_STORAGE_KEYS.CONTACTS);
    mockService.set(LOCAL_STORAGE_KEYS.CONTACTS, all.filter((msg: any) => msg.id !== id));
  }
};
...
export const authService = {
  register: async (name: string, email: string, password: string): Promise<boolean> => {
    ...
    const newUser: User = { id: Math.random().toString(36).substr(2, 9), name, email, role: 'user', created_at: new Date().toISOString() };
    mockService.set(LOCAL_STORAGE_KEYS.USERS, [...users, { ...newUser, password }]);
    return true;
  },
  login: async (email: string, password: string): Promise<boolean> => {
    if (email === 'wilka' && password === 'wilka') {
      const adminSession: User = { id: 'admin-1', name: 'WilkaXyz', email: 'wilka@smkn2.id', role: 'admin', created_at: new Date().toISOString() };
  ...
  logout: async () => {
    ...
    sessionStorage.removeItem('v_tracked');
  },
  ...
  isCloudActive: (): boolean => !!supabase,
